const question9El = [
    {
      id: "1a",
      status: "Capitals",
      country: "Tάλιν",
      image: require("../../../assets/Flags/estonia.png"),
      options: [
        {
          id: "0",
          capital: "Λευκορωσία",
          country: "Mινσκ",
        },
        {
          id: "1",
          capital: "Λετονία",
          country: "Ρίγα",
        },
        {
          id: "2",
          capital: "Eσθονία",
          country: "Tάλιν",
        },
        {
          id: "3",
          capital: "Λιθουανία",
          country: "Βίλνιους",
        },
      ],
      correctAnswerIndex: 2,
    },
    {
      id: "1b",
      status: "Flags",
      capital: "Tάλιν",
      country: "Eσθονία",
      options: [
        {
          id: "0",
          img: require("../../../assets/Flags/finland.png"),
          country: "Φινλανδία",
        },
        {
          id: "1",
          img: require("../../../assets/Flags/kazakhstan.png"),
          country: "Kαζακστάν",
        },
        {
          id: "2",
          img: require("../../../assets/Flags/estonia.png"),
          country: "Eσθονία",
        },
        {
          id: "3",
          img: require("../../../assets/Flags/azerbaijan.png"),
          country: "Mοζαμβίκη",
        },
      ],
      correctAnswerIndex: 2,
    },
    {
      id: "1c",
      status: "Guess",
      word: "ΕΣΘΟΝΙΑ",
      country: "Tάλιν",
      num: 7,
      letters: "ΕΠΟΑΚΛΝΙΣΘΝΜ",
      img: require("../../../assets/Flags/estonia.png"),
    },
    {
      id: "2a",
      status: "Capitals",
      country: "Aστάνα",
      image: require("../../../assets/Flags/kazakhstan.png"),
      options: [
        {
          id: "0",
          capital: 'Aρμενία',
          country: 'Γερεβάν'
        },
        {
          id: "1",
          capital: 'Kαζακστάν',
          country: 'Aστανά'
        },
        {
          id: "2",
          capital: 'Ουζμπεκιστάν',
          country: 'Tασκένδη'
        },
        {
          id: "3",
          capital: 'Kιργιστάν',
          country: 'Μπισκέκ'
        },
      ],
      correctAnswerIndex: 1,
    },
    {
      id: "2b",
      status: "Flags",
      capital: "Aστάνα",
      country: "Kαζακστάν",
      options: [
        {
          id: "0",
          img: require("../../../assets/Flags/Rwanda.png"),
          country: "Ρουάντα",
        },
        {
          id: "1",
          img: require("../../../assets/Flags/kazakhstan.png"),
          country: "Καζακστάν",
        },
        {
          id: "2",
          img: require("../../../assets/Flags/azerbaijan.png"),
          country: "Aζερμπαϊτζάν",
        },
        {
          id: "3",
          img: require("../../../assets/Flags/Palau.png"),
          country: "Παλάου",
        },
        
      ],
      correctAnswerIndex: 1,
    },
    {
      id: "2c",
      status: "Guess",
      word: "ΚΑΖΑΚΣΤΑΝ",
      country: "Aστάνα",
      num: 6,
      letters: "ΠΟΑΣΡΤΖΚΛΝΕΙ",
      img: require("../../../assets/Flags/kazakhstan.png"),
    },
    {
      id: "3a",
      status: "Capitals",
      country: "Μπανγκόκ",
      image: require("../../../assets/Flags/thailand.png"),
      options: [
        {
          id: "0",
          capital: 'Καμπότζη',
          country: 'Πνομ Πεν'
        },
        {
          id: "1",
          capital: 'Mογγολία',
          country: 'Ουλάν Μπατόρ'
        },
        {
          id: "2",
          capital: 'Λάος',
          country: 'Βιεντιάν'
        },
        {
          id: "3",
          capital: 'Ταϊλάνδη',
          country: 'Μπανγκόκ'
        },
      ],
      correctAnswerIndex: 3,
    },
  
    {
      id: "3b",
      status: "Flags",
      capital: "Μπανγκόκ",
      country: "Tαϊλάνδη",
      options: [
        {
          id: "0",
          img: require("../../../assets/Flags/costaRica.png"),
          country: "Κόστα Ρίκα",
        },
        {
          id: "1",
          img: require("../../../assets/Flags/Cambodia.png"),
          country: "Καμπότζη",
        },
        {
          id: "2",
          img: require("../../../assets/Flags/dominican.png"),
          country: "Δομινικανή Δημοκρατία",
        },
        {
          id: "3",
          img: require("../../../assets/Flags/thailand.png"),
          country: "Tαϊλάνδη",
        },
      ],
      correctAnswerIndex: 3,
    },
    {
      id: "3c",
      status: "Guess",
      word: "ΤΑΪΛΑΝΔΗ",
      country: "Μπανγκόκ",
      num: 7,
      letters: "ΑΣΛΔΝΗΤΪΠΟΤΕ",
      img: require("../../../assets/Flags/thailand.png"),
    },
    {
      id: "4a",
      status: "Capitals",
      country: "Σούβα",
      image: require("../../../assets/Flags/Fiji.png"),
      options: [
        {
          id: "0",
          capital: 'Σαμόα',
          country: 'Απία'
        },
        {
          id: "1",
          capital: 'Φίτζι',
          country: 'Σούβα'
        },
        {
          id: "2",
          capital: 'Παπούα Νέα\nΓουινέα',
          country: 'Πορτ Μόρεσμπι'
        },
        {
          id: "3",
          capital: 'Νησιά Σολομώντα',
          country: 'Χονιάρα'
        },
      ],
      correctAnswerIndex: 1,
    },
    {
      id: "4b",
      status: "Flags",
      capital: "Σούβα",
      country: "Φίτζι",
      options: [
        {
          id: "0",
          img: require("../../../assets/Flags/Solomon-Islands.png"),
          country: "Νησιά Σολομώντα",
        },
        {
          id: "1",
          img: require("../../../assets/Flags/Fiji.png"),
          country: "Φίτζι",
        },
        {
          id: "2",
          img: require("../../../assets/Flags/Papua-New-Guinea.png"),
          country: "Παπούα Νέα\nΓουινέα",
        },
        {
          id: "3",
          img: require("../../../assets/Flags/Marshall-Islands.png"),
          country: "Νησιά Μάρσαλ",
        },
      ],
      correctAnswerIndex: 1,
    },
    {
      id: "4c",
      status: "Guess",
      word: "ΦΙΤΖΙ",
      country: "Σούβα",
      num: 4,
      letters: "ΠΟΑΣΛΚΡΤΦΝΙΖ",
      img: require("../../../assets/Flags/Fiji.png"),
    },
    {
      id: "5a",
      status: "Capitals",
      country: "Μπακού",
      image: require("../../../assets/Flags/azerbaijan.png"),
      options: [
        {
          id: "0",
          capital: 'Αφγανιστάν',
          country: 'Καμπούλ'
        },
        {
          id: "1",
          capital: 'Τατζικιστάν',
          country: 'Ντουσαμπέ'
        },
        {
          id: "2",
          capital: 'Αζερμπαϊτζάν',
          country: 'Μπακού'
        },
        {
          id: "3",
          capital: 'Kαζακστάν',
          country: 'Aστάνα'
        },
      ],
      correctAnswerIndex: 2,
    },
    {
      id: "5b",
      status: "Flags",
      capital: "Μπακού",
      country: "Aζερμπαϊτζάν",
      options: [
        {
          id: "0",
          img: require("../../../assets/Flags/azerbaijan.png"),
          country: "Aζερμπαϊτζάν",
        },
        {
          id: "1",
          img: require("../../../assets/Flags/Algeria.png"),
          country: "Αλγερία",
        },
        {
          id: "2",
          img: require("../../../assets/Flags/kazakhstan.png"),
          country: "Καζακστάν",
        },
        {
          id: "3",
          img: require("../../../assets/Flags/Rwanda.png"),
          country: "Ρουάντα",
        },
      ],
      correctAnswerIndex: 0,
    },
    {
      id: "5c",
      status: "Guess",
      word: "ΑΖΕΡΜΠΑΪΤΖΑΝ",
      country: "Μπακού",
      num: 9,
      letters: "ΖΤΠΟΕΚΛΑΜΝΡΪ",
      img: require("../../../assets/Flags/azerbaijan.png"),
    },
    {
      id: "6a",
      status: "Capitals",
      country: "Αλγέρι",
      image: require("../../../assets/Flags/Algeria.png"),
      options: [
        {
          id: "0",
          capital: 'Λιβύη',
          country: 'Tρίπολη'
        },
        {
          id: "1",
          capital: 'Tυνησία',
          country: 'Tύνιδα'
        },
        {
          id: "2",
          capital: 'Αίγυπτος',
          country: 'Κάιρο'
        },
        {
          id: "3",
          capital: 'Aλγερία',
          country: 'Aλγέρι'
        },
      ],
      correctAnswerIndex: 3,
    },
    {
      id: "6b",
      status: "Flags",
      capital: "Aλγέρι",
      country: "Aλγερία",
      options: [
        {
          id: "0",
          img: require("../../../assets/Flags/azerbaijan.png"),
          country: "Αζερμπαϊτζάν",
        },
        {
          id: "1",
          img: require("../../../assets/Flags/libya.png"),
          country: "Λιβύη",
        },
        {
          id: "2",
          img: require("../../../assets/Flags/pakistan.png"),
          country: "Πακιστάν",
        },
        {
          id: "3",
          img: require("../../../assets/Flags/Algeria.png"),
          country: "Aλγερία",
        },
      ],
      correctAnswerIndex: 3,
    },
    {
      id: "6c",
      status: "Guess",
      word: "ΑΛΓΕΡΙΑ",
      country: "Aλγέρι",
      num: 6,
      letters: "ΑΣΠΟΕΡΓΜΝΛΙΥ",
      img: require("../../../assets/Flags/Algeria.png"),
    },
    {
      id: "7a",
      status: "Capitals",
      country: "Γιαουντέ",
      image: require("../../../assets/Flags/Cameroon.png"),
      options: [
        {
          id: "0",
          capital: 'Καμερούν',
          country: 'Γιαουντέ'
        },
        {
          id: "1",
          capital: 'Nιγηρία',
          country: 'Aμπούτζα'
        },
        {
          id: "2",
          capital: 'Aνγκόλα',
          country: 'Λουάντα'
        },
        {
          id: "3",
          capital: 'Zάμπια',
          country: 'Λουσάκα'
        },
      ],
      correctAnswerIndex: 0,
    },
    {
      id: "7b",
      status: "Flags" ,
      capital: "Γιαουντέ",
      country: "Καμερούν",
      options: [
        {
          id: "0",
          img: require("../../../assets/Flags/Cameroon.png"),
          country: 'Καμερούν'
        },
        {
          id: "1",
          img: require("../../../assets/Flags/senegal.png"),
          country: 'Σενεγάλη'
        },
        {
          id: "2",
          img: require("../../../assets/Flags/bolivia.png"),
          country: 'Βολιβία'
        },
        {
          id: "3",
          img: require("../../../assets/Flags/lithuania.png"),
          country: 'Λιθουανία'
        },
      ],
      correctAnswerIndex: 0,
    },
    {
      id: "7c",
      status: "Guess",
      word: "ΚΑΜΕΡΟΥΝ",
      country: "Γιαουντέ",
      num: 8,
      letters: "ΠΟΑΣΡΝΥΚΜΕΙΔ",
      img: require("../../../assets/Flags/Cameroon.png"),
    },
    {
      id: "8a",
      status: "Capitals",
      country: "Κίνγκστον",
      image: require("../../../assets/Flags/jamaica.png"),
      options: [
        {
          id: "0",
          capital: 'Αϊτή',
          country: 'Πορτ-ο-Πρενς'
        },
        {
          id: "1",
          capital: 'Τζαμάικα',
          country: 'Kίνγκστον'
        },
        {
          id: "2",
          capital: 'Μπαχάμες',
          country: 'Nασσάου'
        },
        {
          id: "3",
          capital: 'Πουέρτο Ρίκο',
          country: 'Σαν Χουάν'
        },
      ],
      correctAnswerIndex: 1,
    },
    {
      id: "8b",
      status: "Flags",
      capital: "Κίνγκστον",
      country: "Τζαμάικα",
      options: [
        {
          id: "0",
          img: require("../../../assets/Flags/Mozambique.png"),
          country: "Μοζαμβίκη",
        },
        {
          id: "1",
          img: require("../../../assets/Flags/jamaica.png"),
          country: "Τζαμάικα",
        },
        {
          id: "2",
          img: require("../../../assets/Flags/Solomon-Islands.png"),
          country: "Νησιά Σολομώντα",
        },
        {
          id: "3",
          img: require("../../../assets/Flags/Zimbabwe.png"),
          country: "Zιμπάμπουε",
        },
      ],
      correctAnswerIndex: 1,
    },
    {
      id: "8c",
      status: "Guess",
      word: "ΤΖΑΜΑΙΚΑ",
      country: "Κίνγκστον",
      num: 6,
      letters: "ΠΟΑΣΝΙΜΤΕΥΖΚ",
      img: require("../../../assets/Flags/jamaica.png"),
    },
    {
      id: "9a",
      status: "Capitals",
      country: "Mουσκάτ",
      image: require("../../../assets/Flags/Oman.png"),
      options: [
        {
          id: "0",
          capital: 'Κουβέιτ',
          country: 'Κουβέιτ'
        },
        {
          id: "1",
          capital: 'Σαουδική Αραβία',
          country: 'Ριάντ'
        },
        {
          id: "2",
          capital: 'Υεμένη',
          country: "Σανάα"
        },
        {
          id: "3",
          capital: 'Ομάν',
          country: 'Mουσκάτ'
        },
      ],
      correctAnswerIndex: 3,
    },
    {
      id: "9b",
      status: "Flags",
      capital: "Μουσκάτ",
      country: "Oμάν",
      options: [
        {
          id: "0",
          img: require("../../../assets/Flags/Kuwait.png"),
          country: "Κουβέιτ",
        },
        {
          id: "1",
          img: require("../../../assets/Flags/United-Arab-Emirates.png"),
          country: "Ηνωμένα\nΑραβικά Εμιράτα",
        },
        {
          id: "2",
          img: require("../../../assets/Flags/Madagascar.png"),
          country: "Μαδαγασκάρη",
        },
        {
          id: "3",
          img: require("../../../assets/Flags/Oman.png"),
          country: "Mουσκάτ",
        },
      ],
      correctAnswerIndex: 3,
    },
    {
      id: "9c",
      status: "Guess",
      word: "ΟΜΑΝ",
      country: "Mουσκάτ",
      num: 4,
      letters: "ΠΟΑΣΤΜΝΕΒΚΙΥ",
      img: require("../../../assets/Flags/Oman.png"),
    },
    {
      id: "10a",
      status: "Capitals",
      country: "Βηρυτός",
      image: require("../../../assets/Flags/lebanon.png"),
      options: [
        {
          id: "0",
          capital: 'Συρία',
          country: 'Δαμασκός'
        },
        {
          id: "1",
          capital: 'Λίβανος',
          country: 'Βηρυτός'
        },
        {
          id: "2",
          capital: 'Λιβύη',
          country: 'Tρίπολη'
        },
        {
          id: "3",
          capital: 'Ιορδανία',
          country: 'Aμμάν'
        },
      ],
      correctAnswerIndex: 1,
    },
    {
      id: "10b",
      status: "Flags",
      capital: "Βηρυτός",
      country: "Λίβανος",
      options: [
        {
          id: "0",
          img: require("../../../assets/Flags/peru.png"),
          country: "Περού",
        },
        {
          id: "1",
          img: require("../../../assets/Flags/lebanon.png"),
          country: "Λίβανος",
        },
        {
          id: "2",
          img: require("../../../assets/Flags/Moldova.png"),
          country: "Μολδαβία",
        },
        {
          id: "3",
          img: require("../../../assets/Flags/estonia.png"),
          country: "Eσθονία",
        },
      ],
      correctAnswerIndex: 1,
    },
    {
      id: "10c",
      status: "Guess",
      word: "ΛΙΒΑΝΟΣ",
      country: "Βηρυτός",
      num: 7,
      letters: "ΠΟΑΣΛΙΥΒΝΕΤΜ",
      img: require("../../../assets/Flags/lebanon.png"),
    },
    {
      id: "11",
      status: "Flags",
      capital: "",
      country: "",
    },
  ];
  export default question9El;
  
const question6El = [
  {
    id: "1a",
    status: "Capitals",
    country: "Βουδαπέστη",
    image: require("../../../assets/Flags/hungary.png"),
    options: [
      {
        id: "0",
        capital: 'Ουγγαρία',
        country: 'Bουδαπέστη'
      },
      {
        id: "1",
        capital: 'Ρουμανία',
        country: 'Bουκουρέστι'
      },
      {
        id: "2",
        capital: 'Mολδαβία',
        country: 'Κισινάου'
      },
      {
        id: "3",
        capital: 'Ουκρανία',
        country: 'Κίεβο'
      },
    ],
    correctAnswerIndex: 0,
    phoneHelp: ''
  },
  {
    id: "1b",
    status: "Flags",
    capital: "Bουδαπέστη",
    country: "Ουγγαρία",
    options: [
      {
        id: "0",
        img: require("../../../assets/Flags/hungary.png"),
        country: "Ουγγαρία",
      },
      {
        id: "1",
        img: require("../../../assets/Flags/italy.png"),
        country: "Iταλία",
      },
      {
        id: "2",
        img: require("../../../assets/Flags/iran.png"),
        country: "Iράν",
      },
      {
        id: "3",
        img: require("../../../assets/Flags/india.png"),
        country: "Ινδία",
      },
    ],
    correctAnswerIndex: 0,
    phoneHelp: ''
  },
  {
    id: "1c",
    status: "Guess",
    word: "ΟΥΓΓΑΡΙΑ",
    country: "Bουδαπέστη",
    num: 6,
    letters: "ΠΟΓΕΡΙΑΜΝΚΣΥ",
    img: require("../../../assets/Flags/hungary.png"),
  },
  {
    id: "2a",
    status: "Capitals",
    country: "Ουέλλινγκτον",
    image: require("../../../assets/Flags/newZealand.png"),
    options: [
      {
        id: "0",
        capital: 'Ινδονησία',
        country: 'Τζακάρτα'
      },
      {
        id: "1",
        capital: 'Aυστραλία',
        country: 'Καμπέρα'
      },
      {
        id: "2",
        capital: 'Mαλαισία',
        country: 'Kουάλα Λουμπούρ'
      },
      {
        id: "3",
        capital: 'Νέα Ζηλανδία',
        country: 'Ουέλλινγκτον'
      },
    ],
    correctAnswerIndex: 3,
    phoneHelp: ''
  },
  {
    id: "2b",
    status: "Flags",
    capital: "Ουέλλινγκτον",
    country: "Νέα Ζηλανδία",
    options: [
      {
        id: "0",
        img: require("../../../assets/Flags/australia.png"),
        country: "Αυστραλία",
      },
      {
        id: "1",
        img: require("../../../assets/Flags/Fiji.png"),
        country: "Φίτζι",
      },
      {
        id: "2",
        img: require("../../../assets/Flags/Solomon-Islands.png"),
        country: "Νησιά Σολομώντα",
      },
      {
        id: "3",
        img: require("../../../assets/Flags/newZealand.png"),
        country: "Νέα Ζηλανδία",
      },
    ],
    correctAnswerIndex: 3,
    phoneHelp: ''
  },
  {
    id: "2c",
    status: "Guess",
    word: "ΝΕΑΖΗΛΑΝΔΙΑ",
    country: "Ουέλλνγκτον",
    num: 8,
    letters: "ΠΟΑΣΛΙΝΔΖΗΚΕ",
    img: require("../../../assets/Flags/newZealand.png"),
  },
  {
    id: "3a",
    status: "Capitals",
    country: "Ισλαμαμπάντ",
    image: require("../../../assets/Flags/pakistan.png"),
    options: [
      {
        id: "0",
        capital: 'Μπαγκλαντές',
        country: 'Ντάκα'
      },
      {
        id: "1",
        capital: 'Πακιστάν',
        country: 'Ισλαμαμπάντ'
      },
      {
        id: "2",
        capital: 'Iνδία',
        country: 'Nέο Δελχί'
      },
      {
        id: "3",
        capital: 'Nεπάλ',
        country: 'Kατμαντού'
      },
    ],
    correctAnswerIndex: 1,
    phoneHelp: ''
  },

  {
    id: "3b",
    status: "Flags",
    capital: "Iσλαμαμπάντ",
    country: "Πακιστάν",
    options: [
      {
        id: "0",
        img: require("../../../assets/Flags/Algeria.png"),
        country: "Aλγερία",
      },
      {
        id: "1",
        img: require("../../../assets/Flags/pakistan.png"),
        country: "Πακιστάν",
      },
      {
        id: "2",
        img: require("../../../assets/Flags/kazakhstan.png"),
        country: "Καζακστάν",
      },
      {
        id: "3",
        img: require("../../../assets/Flags/azerbaijan.png"),
        country: "Aζερμπαϊτζάν",
      },
    ],
    correctAnswerIndex: 1,
    phoneHelp: ''
  },
  {
    id: "3c",
    status: "Guess",
    word: "ΠΑΚΙΣΤΑΝ",
    country: "Iσλαμαμπάντ",
    num: 7,
    letters: "ΑΣΜΝΠΟΕΡΙΚΣΤ",
    img: require("../../../assets/Flags/pakistan.png"),
  },
  {
    id: "4a",
    status: "Capitals",
    country: "Καράκας",
    image: require("../../../assets/Flags/venezuela.png"),
    options: [
      {
        id: "0",
        capital: 'Κολομβία',
        country: 'Μπογκοτά'
      },
      {
        id: "1",
        capital: 'Ισημερινός',
        country: 'Κίτο'
      },
      {
        id: "2",
        capital: 'Βενεζουέλα',
        country: 'Καράκας'
      },
      {
        id: "3",
        capital: 'Περού',
        country: 'Λίμα'
      },
    ],
    correctAnswerIndex: 2,
    phoneHelp: ''
  },
  {
    id: "4b",
    status: "Flags",
    capital: "Καράκας",
    country: "Βενεζουέλα",
    options: [
      {
        id: "0",
        img: require("../../../assets/Flags/mexico.png"),
        country: "Mεξικό",
      },
      {
        id: "1",
        img: require("../../../assets/Flags/brazil.png"),
        country: "Bραζιλία",
      },
      {
        id: "2",
        img: require("../../../assets/Flags/venezuela.png"),
        country: "Βενεζουέλα",
      },
      {
        id: "3",
        img: require("../../../assets/Flags/Moldova.png"),
        country: "Mολδαβία",
      },
    ],
    correctAnswerIndex: 2,
    phoneHelp: ''
  },
  {
    id: "4c",
    status: "Guess",
    word: "ΒΕΝΕΖΟΥΕΛΑ",
    country: "Καράκας",
    num: 8,
    letters: "ΖΟΕΒΝΥΛΑΝΚΠΙ",
    img: require("../../../assets/Flags/venezuela.png"),
  },
  {
    id: "5a",
    status: "Capitals",
    country: "Λιουμπλιάνα",
    image: require("../../../assets/Flags/slovenia.png"),
    options: [
      {
        id: "0",
        capital: 'Aυστρία',
        country: 'Βιέννη'
      },
      {
        id: "1",
        capital: 'Σλοβενία',
        country: 'Λιουμπλιάνα'
      },
      {
        id: "2",
        capital: 'Σλοβακία',
        country: 'Μπρατισλάβα'
      },
      {
        id: "3",
        capital: 'Τσεχία',
        country: 'Πράγα'
      },
    ],
    correctAnswerIndex: 1,
    phoneHelp: ''
  },
  {
    id: "5b",
    status: "Flags",
    capital: "Λιουμπλιάνα",
    country: "Σλοβενία",
    options: [
      {
        id: "0",
        img: require("../../../assets/Flags/colombia.png"),
        country: "Κολομβία",
      },
      {
        id: "1",
        img: require("../../../assets/Flags/slovenia.png"),
        country: "Σλοβενία",
      },
      {
        id: "2",
        img: require("../../../assets/Flags/ireland.png"),
        country: "Ιρλανδία",
      },
      {
        id: "3",
        img: require("../../../assets/Flags/greece.png"),
        country: "Ελλάδα",
      },
    ],
    correctAnswerIndex: 1,
    phoneHelp: ''
  },
  {
    id: "5c",
    status: "Guess",
    word: "ΣΛΟΒΕΝΙΑ",
    country: "Λιουμπλιάνα",
    num: 8,
    letters: "ΑΣΠΟΕΡΚΛΤΝΒΙ",
    img: require("../../../assets/Flags/slovenia.png"),
  },
  {
    id: "6a",
    status: "Capitals",
    country: "Νέο Δελχί",
    image: require("../../../assets/Flags/india.png"),
    options: [
      {
        id: "0",
        capital: 'Πακιστάν',
        country: 'Iσλαμαμπάντ'
      },
      {
        id: "1",
        capital: 'Νεπάλ',
        country: 'Kατμαντού'
      },
      {
        id: "2",
        capital: 'Mογγολία',
        country: 'Ουλάν Μπατόρ'
      },
      {
        id: "3",
        capital: 'Iνδία',
        country: 'Nέο Δελχί'
      },
    ],
    correctAnswerIndex: 3,
    phoneHelp: ''
  },
  {
    id: "6b",
    status: "Flags",
    capital: "Nέο Δελχί",
    country: "Iνδία",
    options: [
      {
        id: "0",
        img: require("../../../assets/Flags/ireland.png"),
        country: "Iρλανδία",
      },
      {
        id: "1",
        img: require("../../../assets/Flags/mexico.png"),
        country: "Mεξικό",
      },
      {
        id: "2",
        img: require("../../../assets/Flags/iran.png"),
        country: "Iράν",
      },
      {
        id: "3",
        img: require("../../../assets/Flags/india.png"),
        country: "Iνδία",
      },
    ],
    correctAnswerIndex: 3,
    phoneHelp: ''
  },
  {
    id: "6c",
    status: "Guess",
    word: "ΙΝΔΙΑ",
    country: "Nέο Δελχί",
    num: 4,
    letters: "ΠΟΙΑΣΝΔΛΚΕΤΥ",
    img: require("../../../assets/Flags/india.png"),
  },
  {
    id: "7a",
    status: "Capitals",
    country: "Σαν Χοσέ",
    image: require("../../../assets/Flags/costaRica.png"),
    options: [
      {
        id: "0",
        capital: 'Κούβα',
        country: 'Αβάνα'
      },
      {
        id: "1",
        capital: 'Κόστα Ρίκα',
        country: 'Σαν Χοσέ'
      },
      {
        id: "2",
        capital: 'Πουέρτο Ρίκο',
        country: 'Σαν Χουάν'
      },
      {
        id: "3",
        capital: 'Χιλή',
        country: 'Σαντιάγο'
      },
    ],
    correctAnswerIndex: 1,
    phoneHelp: ''
  },
  {
    id: "7b",
    status: "Flags" ,
    capital: "Σαν Χοσέ",
    country: "Κόστα Ρίκα",
    options: [
      {
        id: "0",
        img: require("../../../assets/Flags/Panama.png"),
        country: 'Παναμάς'
      },
      {
        id: "1",
        img: require("../../../assets/Flags/costaRica.png"),
        country: 'Κόστα Ρίκα'
      },
      {
        id: "2",
        img: require("../../../assets/Flags/thailand.png"),
        country: 'Ταϊλάνδη'
      },
      {
        id: "3",
        img: require("../../../assets/Flags/dominican.png"),
        country: 'Δομινικανή Δημοκρατία'
      },
    ],
    correctAnswerIndex: 1,
    phoneHelp: ''
  },
  {
    id: "7c",
    status: "Guess",
    word: "ΚΟΣΤΑΡΙΚΑ",
    country: "Σαν Χοσέ",
    num: 7,
    letters: "ΠΟΑΣΤΚΕΡΙΛΝΥ",
    img: require("../../../assets/Flags/costaRica.png"),
  },
  {
    id: "8a",
    status: "Capitals",
    country: "Ριάντ",
    image: require("../../../assets/Flags/saudi-arabia.png"),
    options: [
      {
        id: "0",
        capital: 'Μπαχρέιν',
        country: 'Μανάμα'
      },
      {
        id: "1",
        capital: 'Υεμένη',
        country: 'Σαναά'
      },
      {
        id: "2",
        capital: 'Σαουδική Αραβία',
        country: 'Ριάντ'
      },
      {
        id: "3",
        capital: 'Ηνωμένα Αραβικά \nΕμιράτα',
        country: 'Άμπου Ντάμπι'
      },
    ],
    correctAnswerIndex: 2,
    phoneHelp: ''
  },
  {
    id: "8b",
    status: "Flags",
    capital: "Ριάντ",
    country: "Σαουδική Αραβία",
    options: [
      {
        id: "0",
        img: require("../../../assets/Flags/nigeria.png"),
        country: "Nιγηρία",
      },
      {
        id: "1",
        img: require("../../../assets/Flags/Oman.png"),
        country: "Oμάν",
      },
      {
        id: "2",
        img: require("../../../assets/Flags/saudi-arabia.png"),
        country: "Σαουδική Αραβία",
      },
      {
        id: "3",
        img: require("../../../assets/Flags/kazakhstan.png"),
        country: "Καζακστάν",
      },
    ],
    correctAnswerIndex: 2,
    phoneHelp: ''
  },
  {
    id: "8c",
    status: "Guess",
    word: "ΣΑΟΥΔΙΚΗΑΡΑΒΙΑ",
    country: "Ριάντ",
    num: 10,
    letters: "ΠΟΣΑΙΡΒΚΥΔΗΛ",
    img: require("../../../assets/Flags/saudi-arabia.png"),
  },
  {
    id: "9a",
    status: "Capitals",
    country: "Σαν Χουάν",
    image: require("../../../assets/Flags/puertoRico.jpg"),
    options: [
      {
        id: "0",
        capital: 'Πουέρτο Ρίκο',
        country: 'Σαν Χουάν'
      },
      {
        id: "1",
        capital: 'Αϊτή',
        country: 'Πορτ-ο-Πρενς'
      },
      {
        id: "2",
        capital: 'Τζαμάικα',
        country: 'Kίνγκστον'
      },
      {
        id: "3",
        capital: 'Nικαράγουα',
        country: 'Mανάγουα'
      },
    ],
    correctAnswerIndex: 0,
    phoneHelp: ''
  },
  {
    id: "9b",
    status: "Flags",
    capital: "Σαν Χουάν",
    country: "Πουέρτο Ρίκο",
    options: [
      {
        id: "0",
        img: require("../../../assets/Flags/puertoRico.jpg"),
        country: "Πουέρτο Ρίκο",
      },
      {
        id: "1",
        img: require("../../../assets/Flags/czechia.png"),
        country: "Τσεχία",
      },
      {
        id: "2",
        img: require("../../../assets/Flags/chile.png"),
        country: "Χιλή",
      },
      {
        id: "3",
        img: require("../../../assets/Flags/Cuba.png"),
        country: "Κούβα",
      },
    ],
    correctAnswerIndex: 0,
    phoneHelp: ''
  },
  {
    id: "9c",
    status: "Guess",
    word: "ΠΟΥΕΡΤΟΡΙΚΟ",
    country: "Σαν Χουάν",
    num: 8,
    letters: "ΠΟΑΣΡΥΙΕΚΤΜΝ",
    img: require("../../../assets/Flags/puertoRico.jpg"),
  },
  {
    id: "10a",
    status: "Capitals",
    country: "Ασουνσιόν",
    image: require("../../../assets/Flags/paraguay.png"),
    options: [
      {
        id: "0",
        capital: 'Ουρουγουάη',
        country: 'Mοντεβιδέο'
      },
      {
        id: "1",
        capital: 'Παραγουάη',
        country: 'Aσουνσιόν'
      },
      {
        id: "2",
        capital: 'Αργεντινή',
        country: 'Μπουένος Άιρες'
      },
      {
        id: "3",
        capital: 'Χιλή',
        country: 'Σαντιάγο'
      },
    ],
    correctAnswerIndex: 1,
    phoneHelp: ''
  },
  {
    id: "10b",
    status: "Flags",
    capital: "Ασουνσιόν",
    country: "Παραγουάη",
    options: [
      {
        id: "0",
        img: require("../../../assets/Flags/netherlands.png"),
        country: "Ολλανδία",
      },
      {
        id: "1",
        img: require("../../../assets/Flags/paraguay.png"),
        country: "Παραγουάη",
      },
      {
        id: "2",
        img: require("../../../assets/Flags/bulgaria.png"),
        country: "Bουλγαρία",
      },
      {
        id: "3",
        img: require("../../../assets/Flags/peru.png"),
        country: "Περού",
      },
    ],
    correctAnswerIndex: 1,
    phoneHelp: ''
  },
  {
    id: "10c",
    status: "Guess",
    word: "ΠΑΡΑΓΟΥΑΗ",
    country: "Ασουνσιόν",
    num: 7,
    letters: "ΡΗΑΚΛΠΥΓΟΜΝΕ",
    img: require("../../../assets/Flags/paraguay.png"),
  },
  {
    id: "11",
    status: "Flags",
    capital: "",
    country: "",
  },
];
export default question6El;

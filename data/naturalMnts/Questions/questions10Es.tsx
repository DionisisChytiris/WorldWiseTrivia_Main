const questions10Es = [
  {
    id: "1a",
    status: "Capitals",
    img: 'L',
    country: "Grecia",
    image: require("../../../assets/WorldMonuments/Europe/meteora.jpg"),
    options: [
      {
        id: "0",
        capital: "Cabo de Buena Esperanza",
        country: "Sudáfrica",
      },
      {
        id: "1",
        capital: "Nilo",
        country: "Egipto",
      },
      {
        id: "2",
        capital: "Meteora",
        country: "Grecia",
      },
      {
        id: "3",
        capital: "Monte Sinaí\nEgipto",
      },
    ],
    correctAnswerIndex: 2,
  },
  {
    id: "1b",
    status: "Flags",
    country: "Meteora",
    capital: "Grecia",
    options: [
      {
        id: "0",
        country: "Cabo de Buena Esperanza\nSudáfrica",
        img: require("../../../assets/NaturalMnt/Africa/CapeofGoodHope.webp"),
      },
      {
        id: "1",
        country: "Nilo\nEgipto",
        img: require("../../../assets/NaturalMnt/Africa/RiverNile.webp"),
      },
      {
        id: "2",
        country: "Meteora\nGrecia",
        img: require("../../../assets/WorldMonuments/Europe/meteora.jpg"),
      },
      {
        id: "3",
        country: "Monte Sinaí\nEgipto",
        img: require("../../../assets/NaturalMnt/Africa/mountSinai.webp"),
      },
    ],
    correctAnswerIndex: 2,
  },
  {
    id: "2a",
    status: "Capitals",
    img: 'L',
    country: "Costa Rica",
    image: require("../../../assets/NaturalMnt/America/Monteverde-cloud-forest-Costa-Rica.webp"),
    options: [
      {
        id: "0",
        capital: "Reserva del Bosque Nuboso de Monteverde",
        country: "Costa Rica",
      },
      {
        id: "1",
        capital: "Pamukale",
        country: "Turquía",
      },
      {
        id: "2",
        capital: "Wadi Rum",
        country: "Jordania",
      },
      {
        id: "3",
        capital: "Monte Kenia",
        country: "Kenia",
      },
    ],
    correctAnswerIndex: 0,
  },
  {
    id: "2b",
    status: "Flags",
    country: "Reserva del Bosque Nuboso de Monteverde",
    capital: "Costa Rica",
    options: [
      {
        id: "0",
        country: "Reserva del Bosque Nuboso de Monteverde\nCosta Rica",
        img: require("../../../assets/NaturalMnt/America/Monteverde-cloud-forest-Costa-Rica.webp"),
      },
      {
        id: "1",
        country: "Wadi Rum\nJordania",
        img: require("../../../assets/NaturalMnt/Asia/wadi-rum.webp"),
      },
      {
        id: "2",
        country: "Monte Kenia\nKenia",
        img: require("../../../assets/NaturalMnt/Africa/mountKenya.webp"),
      },
      {
        id: "3",
        country: "Pamukale\nTurquía",
        img: require("../../../assets/NaturalMnt/Asia/Pamukkale.webp"),
      },
    ],
    correctAnswerIndex: 0,
  },
  {
    id: "3a",
    status: "Capitals",
    img: 'L',
    country: "Tanzania",
    image: require("../../../assets/NaturalMnt/Africa/ngorongo.jpg"),
    options: [
      {
        id: "0",
        capital: "Salto Ángel",
        country: "Venezuela",
      },
      {
        id: "1",
        capital: "Ojo de la Tierra",
        country: "Croacia",
      },
      {
        id: "2",
        capital: "Cráter del Ngorongoro",
        country: "Tanzania",
      },
      {
        id: "3",
        capital: "Meteora",
        country: "Grecia",
      },
    ],
    correctAnswerIndex: 2,
  },
  {
    id: "3b",
    status: "Flags",
    country: "Cráter del Ngorongoro",
    capital: "Tanzania",
    options: [
      {
        id: "0",
        country: "Salto Ángel\nVenezuela",
        img: require("../../../assets/NaturalMnt/America/Angel-Falls.webp"),
      },
      {
        id: "1",
        country: "Ojo de la Tierra\nCroacia",
        img: require("../../../assets/NaturalMnt/Europe/eyelake.jpg"),
      },
      {
        id: "2",
        country: "Cráter del Ngorongoro\nTanzania",
        img: require("../../../assets/NaturalMnt/Africa/ngorongo.jpg"),
      },
      {
        id: "3",
        country: "Meteora\nGrecia",
        img: require("../../../assets/WorldMonuments/Europe/meteora.png"),
      },
    ],
    correctAnswerIndex: 2,
  },
  {
    id: "4a",
    status: "Capitals",
    img: 'L',
    country: "Botswana",
    image: require("../../../assets/NaturalMnt/Africa/Botswana_Okavango_Delta.webp"),
    options: [
      {
        id: "0",
        capital: "Delta del Okavango",
        country: "Botswana",
      },
      {
        id: "1",
        capital: "Puerta del Cielo",
        country: "China",
      },
      {
        id: "2",
        capital: "Monte Bromo",
        country: "Indonesia",
      },
      {
        id: "3",
        capital: "Cueva de Fingal",
        country: "Escocia",
      },
    ],
    correctAnswerIndex: 0,
  },
  {
    id: "4b",
    status: "Flags",
    country: "Delta del Okavango",
    capital: "Botswana",
    options: [
      {
        id: "0",
        country: "Delta del Okavango\nBotswana",
        img: require("../../../assets/NaturalMnt/Africa/Botswana_Okavango_Delta.webp"),
      },
      {
        id: "1",
        country: "Puerta del Cielo\nChina",
        img: require("../../../assets/NaturalMnt/Asia/heavensgate.jpg"),
      },
      {
        id: "2",
        country: "Monte Bromo\nIndonesia",
        img: require("../../../assets/NaturalMnt/Asia/mountBromo.webp"),
      },
      {
        id: "3",
        country: "Cueva de Fingal\nEscocia",
        img: require("../../../assets/NaturalMnt/Europe/Fingalscave.jpg"),
      },
    ],
    correctAnswerIndex: 0,
  },
  {
    id: "5a",
    status: "Capitals",
    img: 'L',
    country: "Croacia",
    image: require("../../../assets/NaturalMnt/Europe/eyelake.jpg"),
    options: [
      {
        id: "0",
        capital: "Calzada del Gigante",
        country: "Irlanda",
      },
      {
        id: "1",
        capital: "Sossusvlei",
        country: "Namibia",
      },
      {
        id: "2",
        capital: "Las Agujas",
        country: "EE.UU.",
      },
      {
        id: "3",
        capital: "Ojo de la Tierra",
        country: "Croacia",
      },
    ],
    correctAnswerIndex: 3,
  },
  {
    id: "5b",
    status: "Flags",
    country: "Ojo de la Tierra",
    capital: "Croacia",
    options: [
      {
        id: "0",
        country: "Calzada del Gigante\nIrlanda",
        img: require("../../../assets/NaturalMnt/Europe/GiantsCauseway.webp"),
      },
      {
        id: "1",
        country: "Las Agujas\nEE.UU.",
        img: require("../../../assets/NaturalMnt/America/TheNeedles.webp"),
      },
      {
        id: "2",
        country: "Sossusvlei\nNamibia",
        img: require("../../../assets/NaturalMnt/Africa/Sossusvlei-Dunes.webp"),
      },
      {
        id: "3",
        country: "Ojo de la Tierra\nCroacia",
        img: require("../../../assets/NaturalMnt/Europe/eyelake.jpg"),
      },
    ],
    correctAnswerIndex: 3,
  },
  {
    id: "6a",
    status: "Capitals",
    img: 'L',
    country: "China",
    image: require("../../../assets/NaturalMnt/Asia/heavensgate.jpg"),
    options: [
      {
        id: "0",
        capital: "Lagos de Plitvice",
        country: "Croacia",
      },
      {
        id: "1",
        capital: "Monte Vesubio",
        country: "Italia",
      },
      {
        id: "2",
        capital: "Puerta del Cielo",
        country: "China",
      },
      {
        id: "3",
        capital: "Uluru",
        country: "Australia",
      },
    ],
    correctAnswerIndex: 2,
  },
  {
    id: "6b",
    status: "Flags",
    country: "Puerta del Cielo",
    capital: "China",
    options: [
      {
        id: "0",
        country: "Monte Vesubio\nItalia",
        img: require("../../../assets/NaturalMnt/Europe/mount-vesuvius.webp"),
      },
      {
        id: "1",
        country: "Lagos de Plitvice\nCroacia",
        img: require("../../../assets/NaturalMnt/Europe/Plitvice-lakes.webp"),
      },
      {
        id: "2",
        country: "Puerta del Cielo\nChina",
        img: require("../../../assets/NaturalMnt/Asia/heavensgate.jpg"),
      },
      {
        id: "3",
        country: "Uluru\nAustralia",
        img: require("../../../assets/NaturalMnt/Oceania/Ayers-Rock.webp"),
      },
    ],
    correctAnswerIndex: 2,
  },
  {
    id: "7a",
    status: "Capitals",
    img: 'L',
    country: "Escocia",
    image: require("../../../assets/NaturalMnt/Europe/Fingalscave.jpg"),
    options: [
      {
        id: "0",
        capital: "Cueva de Fingal",
        country: "Escocia",
      },
      {
        id: "1",
        capital: "Isla de Skye",
        country: "Escocia",
      },
      {
        id: "2",
        capital: "Selva Negra",
        country: "Alemania",
      },
      {
        id: "3",
        capital: "Lago Hillier",
        country: "Australia",
      },
    ],
    correctAnswerIndex: 0,
  },
  {
    id: "7b",
    status: "Flags",
    country: "Cueva de Fingal",
    capital: "Escocia",
    options: [
      {
        id: "0",
        country: "Cueva de Fingal \nEscocia",
        img: require("../../../assets/NaturalMnt/Europe/Fingalscave.jpg"),
      },
      {
        id: "1",
        country: "Isla de Skye\nEscocia",
        img: require("../../../assets/NaturalMnt/Europe/isleofSkye.webp"),
      },
      {
        id: "2",
        country: "Lago Hillier\nAustralia",
        img: require("../../../assets/NaturalMnt/Oceania/lakehillier.webp"),
      },
      {
        id: "3",
        country: "Selva Negra\nAlemania",
        img: require("../../../assets/NaturalMnt/Europe/blackForest.webp"),
      },
    ],
    correctAnswerIndex: 0,
  },
  {
    id: "8a",
    status: "Capitals",
    img: 'L',
    country: "Azerbaiyán",
    image: require("../../../assets/NaturalMnt/Asia/mud-volcanoes-in-gobustan.jpg"),
    options: [
      {
        id: "0",
        capital: "Meteora",
        country: "Grecia",
      },
      {
        id: "1",
        capital: "Monte Teide",
        country: "España",
      },
      {
        id: "2",
        capital: "Volcanes de lodo",
        country: "Azerbaiyán",
      },
      {
        id: "3",
        capital: "Picos de Europa",
        country: "España",
      },
    ],
    correctAnswerIndex: 2,
  },
  {
    id: "8b",
    status: "Flags",
    country: "Volcanes de lodo",
    capital: "Azerbaiyán",
    options: [
      {
        id: "0",
        country: "Meteora\nGrecia",
        img: require("../../../assets/WorldMonuments/Europe/meteora.jpg"),
      },
      {
        id: "1",
        country: "Monte Teide\nEspaña",
        img: require("../../../assets/NaturalMnt/Europe/mountTeide.webp"),
      },
      {
        id: "2",
        country: "Volcanes de lodo\nAzerbaiyán",
        img: require("../../../assets/NaturalMnt/Asia/mud-volcanoes-in-gobustan.jpg"),
      },
      {
        id: "3",
        country: "Picos de Europa\nEspaña",
        img: require("../../../assets/NaturalMnt/Europe/PicosdeEuropa.webp"),
      },
    ],
    correctAnswerIndex: 2,
  },
  {
    id: "9a",
    status: "Capitals",
    img: 'L',
    country: "Nueva Zelanda",
    image: require("../../../assets/NaturalMnt/Oceania/fox-glacier.jpg"),
    options: [
      {
        id: "0",
        capital: "Cañón del Antílope",
        country: "EE.UU.",
      },
      {
        id: "1",
        capital: "Glaciares Fox",
        country: "Nueva Zelanda",
      },
      {
        id: "2",
        capital: "Gran Agujero Azul",
        country: "Belice",
      },
      {
        id: "3",
        capital: "Mesetas Mitten",
        country: "EE.UU.",
      },
    ],
    correctAnswerIndex: 1,
  },
  {
    id: "9b",
    status: "Flags",
    country: "Glaciares Fox",
    capital: "Nueva Zelanda",
    options: [
      {
        id: "0",
        country: "Cañón del Antílope\nEE.UU.",
        img: require("../../../assets/NaturalMnt/America/Antelope-Canyon.webp"),
      },
      {
        id: "1",
        country: "Glaciares Fox\nNueva Zelanda",
        img: require("../../../assets/NaturalMnt/Oceania/fox-glacier.jpg"),
      },
      {
        id: "2",
        country: "Gran Agujero Azul\nBelice",
        img: require("../../../assets/NaturalMnt/America/Great-Blue-Hole.webp"),
      },
      {
        id: "3",
        country: "Mesetas Mitten\nEE.UU.",
        img: require("../../../assets/NaturalMnt/America/mitten-butte.webp"),
      },
    ],
    correctAnswerIndex: 1,
  },
  {
    id: "10a",
    status: "Capitals",
    img: 'L',
    country: "Nueva Zelanda",
    image: require("../../../assets/NaturalMnt/Oceania/NinetyMileBeach.jpg"),
    options: [
      {
        id: "0",
        capital: "Avenidas de Baobabs",
        country: "Madagascar",
      },
      {
        id: "1",
        capital: "Cataratas del Niágara",
        country: "Canadá/EE.UU.",
      },
      {
        id: "2",
        capital: "Playa de las Noventa Millas",
        country: "Nueva Zelanda",
      },
      {
        id: "3",
        capital: "Montaña Arco Iris (Vinicunca)",
        country: "Perú",
      },
    ],
    correctAnswerIndex: 2,
  },
  {
    id: "10b",
    status: "Flags",
    country: "Playa de las Noventa Millas",
    capital: "Nueva Zelanda",
    options: [
      {
        id: "0",
        country: "Avenidas de Baobabs\nMadagascar",
        img: require("../../../assets/NaturalMnt/Africa/avenueofboababs.webp"),
      },
      {
        id: "1",
        country: "Cataratas del Niágara\nCanadá/EE.UU.",
        img: require("../../../assets/NaturalMnt/America/NiagaraFalls.webp"),
      },
      {
        id: "2",
        country: "Playa de las Noventa Millas\nNueva Zelanda",
        img: require("../../../assets/NaturalMnt/Oceania/NinetyMileBeach.jpg"),
      },
      {
        id: "3",
        country: "Montaña Arco Iris (Vinicunca)\nPerú",
        img: require("../../../assets/NaturalMnt/America/rainbowMountain.webp"),
      },
    ],
    correctAnswerIndex: 2,
  },
  {
    id: "11",
    monument: "",
    country: "",
  },
  {
    id: "11a",
    monument: "",
    country: "",
  },
  {
    id: "11s",
    monument: "",
    country: "",
  },
  {
    id: "11d",
    monument: "",
    country: "",
  },
  {
    id: "11f",
    monument: "",
    country: "",
  },
  {
    id: "11g",
    monument: "",
    country: "",
  },
  {
    id: "11h",
    monument: "",
    country: "",
  },
  {
    id: "11j",
    monument: "",
    country: "",
  },
  {
    id: "11k",
    monument: "",
    country: "",
  },
  {
    id: "11l",
    monument: "",
    country: "",
  },
  {
    id: "11m",
    monument: "",
    country: "",
  },
];

export default questions10Es;
